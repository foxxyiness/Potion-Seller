...
namespace Dialogue
{
    ...
    public class DialogueTreeObject : ScriptableObject
    {
        ...
        public void ResetState(int length = 1) 
        {
            if (dialogueState.stateDict[npcName].Length < length)
                return;
            dialogueState.stateDict[npcName] = dialogueState.stateDict[npcName].Remove(dialogueState.stateDict[npcName].Length-length);
